...
// Propósito  : Solicita los datos del ticket Punto de venta para su devolución.
// Creado Por : Juan Navas
// Modificado : Marlon Ramos 17-06-2008 (Permite traer los datos del cliente, el pago y descuentos)
//                           07-07-2008 (Se agrega validación de ticket devuelto al salir del formulario
//                                       evitando generar devolución sobre un ticket más de una vez)
//                           05-08-2008 (Traer los datos de numero y hora para evitar tickets kilométricos)
...
PROCE MAIN(oPos)
   ...
   LOCAL cAsoTipDoc:="" // Tipo de Documento Devolución de Venta
...
//      cAsoTipDoc:=cTipDoc
   ...
   DEFINE DIALOG oDlg TITLE " Devolución de Venta desde ["+cTipDoc+"]" COLOR 0,oDp:nGris2 
   ...
   SAYTEXTO2("Rif o Cédula",oFont,NIL,100,07)
   ...
   SAYTEXTO2("Número de Ticket",oFont,NIL,100,07)
   ...
   IF lOk .AND. lFound .AND. VALTICKET(cTicket)
      ...
      // 06-08-2008 Marlon Ramos (Traer los datos de numero y hora para evitar tickets kilométricos)
      ...
      IF ValType(oPos)="O"
         ...
         // 17-06-2008 Marlon Ramos (Traer el descuento total si lo hay)
            /* 06-08-2008 Marlon Ramos (AGREGAR FECHA Y HORA)
              ...
              // 11-08-2008 Marlon Ramos (Imprimir Fecha, hora y ticket Asociado a la Devolución TICKETEPSON)
...
FUNCTION FINDTICKET(oControl)
   ...
   cNumero:=EJECUTAR("REPBDLIST","DPDOCCLI",{"DOC_NUMERO","IF(CCG_NOMBRE IS NULL,CLI_NOMBRE,CCG_NOMBRE) AS CLI_NOMBRE","DOC_HORA","DOC_NETO"},NIL,;
                     ...
                     " WHERE "+cWhere,;
                     cTitle,;
                     {"Número","Cliente","Hora","Neto"},NIL,NIL,NIL,NIL,oControl)
...
FUNCTION VALTICKET()
   ...
   // Verifica que la Factura no esté Devuelta
   ...
   IF !Empty(cDev)
      MensajeErr("Ticket "+cTicket+", Está Vinculado con Devolución "+cTipDev+" "+;
                 cDev)
      RETURN .F.
...
FUNCTION CargaPagos(cCodSuc,cTipDoc,cTicket)
   ...
   IF oTable:RecCount()=0
      MensajeErr("Documento "+cTipDoc+cTicket+", No se encontró ninguna forma de pago.")
