...
PROCE MAIN(oPos)
  ...
  aData:=ASQL("SELECT GRU_DESCRI,GRU_FILBMP,GRU_CODIGO FROM DPGRU ORDER BY GRU_CODIGO")

  IF EMPTY(aData)
     AADD(aData,{"Ninguno","BITMAPS\GRUPOS.BMP","Ninguno"})
  ENDIF
...
/*
   IF lSelect .OR. oBrw:nLastKey=13
     ...
     oBrw:=NIL
     CursorWait()
     cCodigo:=EJECUTAR("DPPOSSELINV",cGrupo)
   ENDIF
*/
RETURN cCodigo
...
FUNCTION SELPRODUCTO()
   LOCAL cGrupo:=oBrw:aArrayData[oBrw:nArrayAt,3],cTitle:=""
   ...
   cCodigo:=EJECUTAR("DPPOSSELINV","WHERE INV_GRUPO"+GetWhere("=",cGrupo),cGrupo,cTitle)
