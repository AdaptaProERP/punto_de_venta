...
PROCE MAIN(oPos)
  LOCAL nMonto :=12003,lResp:=.F.,oBrw,oCol,oWnd,oGrp,oLetra,oBrush,oRecibe,aTotal:={}
  ...
  LOCAL oVuelto,nVuelto:=0,nVueltoDif:=0
  ...
  LOCAL nVueltoBs := 0,oVueltoBs,nVueltoD,oVueltoBsT

  
// 16770764,16566954

  DEFAULT oDp:cDisp_lGaveta:=""

  aDesglose:={}
  AADD(aDesglose,{0,"X",0,"=",0,.T.})
  ...
  ELSE
    ...
    EJECUTAR("DPPOSCALIGTF","DOL",oPos:nEfePorIGtf)

    oWnd  :=oPos:oWnd
    nMonto:=ROUND((oPos:nNeto+oPos:nIGTF)/oDp:nMonedaExt,2)
/*
...
*/
  ...
  DEFINE BRUSH oBrush FILE "BITMAPS\PAGOEFECTIVODOLAR.BMP"
...
// ErrorSys(.T.)
  ...
  @ 4.6,3 STSAY "Vuelto "+oDp:cMonedaExt;
          RIGHT;
          SIZE 90,10;
          COLORS CLR_BLUE FONT oFont
  ...
  @ 4.6,1.0 STSAY oVuelto PROMPT ALLTRIM(TRAN(nVuelto,"999,999,999.99"));
            OF oDlg;
            COLORS CLR_HGREEN;
            SIZE 100,14;
            FONT oFontB ;
  ...
  @ 7.7,20 STSAY oVueltoBsT PROMPT "Vuelto "+oDp:cMoneda+IF(nVueltoD>0,"",LSTR(nVueltoD));
           RIGHT;
           SIZE 90,10;
           COLORS CLR_BLUE FONT oFont

// PROMPT LSTR(nVueltoDif)+"x"+ALLTRIM(TRAN(oDp:nMonedaExt,"99,999,999,999.99"))+"="+ALLTRIM(TRAN(nVueltoBs,"999,999,999.99"));


  @ 7.5,0 STSAY oVueltoBs PROMPT ALLTRIM(TRAN(nVueltoBs,"999,999,999.99"));
           OF oDlg;
           COLORS CLR_HBLUE;
           SIZE 260,14;
           FONT oFontB ;
  ...
  */
  ...
  oCol:bClrHeader   := {||{CLR_WHITE,CLR_BLUE}}
  oCol:cFooter      :=FDP(aTotal[5],"99,999,999.99")
  ...
  oBrw:bLDblClick:={||oBrw:aArrayData[oBrw:nArrayAt,6]:=!oBrw:aArrayData[oBrw:nArrayAt,6],oBrw:DrawLine(.T.), CALTOTAL() }

  oBrw:bClrStd   := {|nClrText,aLine|aLine   :=oBrw:aArrayData[oBrw:nArrayAt],;
                                     nClrText:=IF(aLine[6],0,6250335),;
  ...
  ACTIVATE DIALOG oDlg ON INIT (oDlg:Move(nTop,nLeft,nWidth,nHeight,.T.),;
                                ...
                                oBrw:Move(15,240,290,nHeight-038-40,.T.),;
                                oVuelto:HIDE())

  IF ValType(oPos)="O" .AND. lResp
    oPos:nEfectivoUs:=nMonto
    oPos:nEfectivo  :=0
    oPos:nRecibe    :=nRecibe
    oPos:nVuelto    :=nVuelto
    oPos:nVueltoD   :=nVuelto
    oPos:nVueltoBs  :=nVueltoBs
    oPos:cMonedaPago:=oDp:cMonedaExt

//  ? oPos:nRecibe,"oPos:nRecibe"
// ? oPos:nVueltoBs,"oPos:nVueltoBs",oPos:nVueltoD,"oPos:nVueltoD",nVuelto,"nVuelto"
...
FUNCTION VUELTO()
  ...
  AEVAL(aDesglose,{|a,n| AADD(aDesglose[n],aDesglose[n,1]>=1)})
...
FUNCTION CALTOTAL()
  ...
  LOCAL aTotal :={},cEnletras
  LOCAL oCol   :=oBrw:aCols[5]
  LOCAL aTotal2:=ATOTALES(oBrw:aArrayData)

  ADEPURA(aData,{|a,n| !a[6]})

  aTotal:=ATOTALES(aData)
  oCol:cFooter      :=FDP(aTotal[5],"99,999,999.99")

  nVueltoDif:=aTotal2[5]-aTotal[5]
  nVueltoBs :=nVueltoDif*oDp:nMonedaExt

// ? nVueltoDif,"nVueltoDif"
