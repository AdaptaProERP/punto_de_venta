...
//   oPosCmd:nColLista :=10 // 12 // Lista de Precios
   ...
   oPosCmd:oMovInv    :=OpenTable("SELECT * FROM DPMOVINV"       ,.F.) // Movimiento Destino Nota de Entrega
   oPosCmd:oDocCli    :=OpenTable("SELECT * FROM DPDOCCLI"       ,.F.) // Documento Destino Nota de Entrega
...
//      oPosCmd:oBrwG:bChange:={||oPosCmd:BRWCHANGEG()}
   ...
   oPosCmd:oBrwM:=TXBrowse():New( oPosCmd:oWnd)
   oPosCmd:oBrwM:SetArray( aDataM, .F. )
   oPosCmd:oBrwM:SetFont(oFont)

   oPosCmd:oBrwM:lFooter     := .T.
   oPosCmd:oBrwM:lHScroll    := .T.
   oPosCmd:oBrwM:nHeaderLines:= 2
   oPosCmd:oBrwM:nDataLines  := 1
   oPosCmd:oBrwM:nFooterLines:= 1
   ...
   AEVAL(oPosCmd:oBrwM:aCols,{|oCol|oCol:oHeaderFont:=oFont})

   oCol:=oPosCmd:oBrwM:aCols[1]
   oCol:cHeader      :='Mesa'+CRLF+"Pedido"
   oCol:bLClickHeader:= {|r,c,f,o| SortArray( o, oPosCmd:oBrwM:aArrayData ) }
   oCol:nWidth       :=80

   oCol:=oPosCmd:oBrwM:aCols[2]
   oCol:cHeader      :='Nombre'
   oCol:bLClickHeader:= {|r,c,f,o| SortArray( o, oPosCmd:oBrwM:aArrayData ) }
   oCol:nWidth       :=80

   oCol:=oPosCmd:oBrwM:aCols[3]
   oCol:cHeader      :='Modo'
   oCol:bLClickHeader:= {|r,c,f,o| SortArray( o, oPosCmd:oBrwM:aArrayData ) }
   oCol:nWidth       :=80


   oCol:=oPosCmd:oBrwM:aCols[4]
   oCol:cHeader      :="Total"+CRLF+"Bs"
   oCol:nWidth       :=60+20
   oCol:bLClickHeader:= {|r,c,f,o| SortArray( o, oPosCmd:oBrwM:aArrayData ) }
   oCol:cFooter      :="0"
   oCol:cEditPicture :='999,999.99'
   oCol:bStrData     :={|nMonto,oCol|nMonto:= oPosCmd:oBrwM:aArrayData[oPosCmd:oBrwM:nArrayAt,4],;
                                     oCol  := oPosCmd:oBrwM:aCols[4],;
                                     FDP(nMonto,oCol:cEditPicture)}

   oCol:=oPosCmd:oBrwM:aCols[5]
   oCol:cHeader      :="Total"+CRLF+"$"
   oCol:nWidth       :=60+20
   oCol:bLClickHeader:= {|r,c,f,o| SortArray( o, oPosCmd:oBrwM:aArrayData ) }
   oCol:cFooter      :="0"
   oCol:cEditPicture :='999,999.99'
   oCol:bStrData     :={|nMonto,oCol|nMonto:= oPosCmd:oBrwM:aArrayData[oPosCmd:oBrwM:nArrayAt,5],;
                                     oCol  := oPosCmd:oBrwM:aCols[5],;
                                     FDP(nMonto,oCol:cEditPicture)}

   oPosCmd:oBrwM:bKeyDown:={|nKey| IF(nKey=46, oPosCmd:BRWDELITEM(),NIL)}


   oPosCmd:oBrwM:bClrStd               := {|oBrw,nClrText,aData|oBrw:=oPosCmd:oBrwM,aData:=oBrw:aArrayData[oBrw:nArrayAt],;
                                           oPosCmd:nClrText,;
                                          {nClrText,iif( oBrw:nArrayAt%2=0, oPosCmd:nClrPane1, oPosCmd:nClrPane2 ) } }

   oPosCmd:oBrwM:bClrHeader            := {|| { oDp:nLbxClrHeaderText, oDp:nLbxClrHeaderPane}}
   oPosCmd:oBrwM:bClrFooter            := {|| { oDp:nLbxClrHeaderText, oDp:nLbxClrHeaderPane}}

   oPosCmd:oBrwM:bLDblClick:={|oBrw|oPosCmd:RUNCLICK() }

   oPosCmd:oBrwM:bChange:={||oPosCmd:BRWCHANGEF()}
   oPosCmd:oBrwM:CreateFromCode()

   oPosCmd:oBrwFocus:=oPosCmd:oBrwM

   oPosCmd:oBrwM:bGotFocus:={||oPosCmd:oBrwFocus:=oPosCmd:oBrwM}
...
//   oPosCmd:oBrw:bChange:={||oPosCmd:BRWCHANGE2()}
   ...
   oPosCmd:oBrwM:Move(250+5+oPosCmd:nAddBar,0   ,400+oPosCmd:nAddCol,400,.T.)
  ...
  @ 50, (83+300+20)-5+oPosCmd:nAddCol  SPLITTER oPosCmd:oSplitV ;
          VERTICAL ;
          PREVIOUS CONTROLS oPosCmd:oBrwM,oPosCmd:oTree,oPosCmd:oSplitH ;
...
FUNCTION ViewDatBar()
   ...
   LOCAL nLin:=0
   LOCAL nWidth:=oPosCmd:oBrwM:nWidth()
   LOCAL nAltoBrw:=150,nCol:=0,nColM:=0
   LOCAL nAdd:=0

   oPosCmd:oWnd:Maximize() // ZOOM
 
   oPosCmd:oBrwM:GoBottom(.T.)
   oPosCmd:oBrwM:Refresh(.T.)
...
//   ENDIF 
 ...
 DEFINE BUTTON oPosCmd:oBtnSave;
   ...
   DEFINE BUTTON oBtn;
          ...
          WHEN LEN(oPosCmd:oBrwM:aArrayData)>1
...
IF nWidth>400
     ...
     DEFINE BUTTON oBtn;
            ...
            TOP PROMPT "Excel"; 
            ACTION  (EJECUTAR("BRWTOEXCEL",oPosCmd:oBrwM,oPosCmd:cTitle,oPosCmd:cNombre))
...
ENDIF
   ...
   DEFINE BUTTON oBtn;
          ...
          FILENAME "BITMAPS\html.BMP";
            TOP PROMPT "Html"; 
              ACTION  (EJECUTAR("BRWTOHTML",oPosCmd:oBrwM))
...
IF nWidth>300
   ...
   DEFINE BUTTON oBtn;
          ...
          FILENAME "BITMAPS\PREVIEW.BMP";
            TOP PROMPT "Preview"; 
              ACTION  (EJECUTAR("BRWPREVIEW",oPosCmd:oBrwM))
...
ENDIF
   ...
   DEFINE BUTTON oBtn;
          ...
          FILENAME "BITMAPS\xTOP.BMP";
            TOP PROMPT "Primero"; 
              ACTION  (oPosCmd:oBrwM:GoTop(),oPosCmd:oBrwM:Setfocus())
...
IF nWidth>800 .OR. nWidth=0
   ...
   DEFINE BUTTON oBtn;
          ...
          FILENAME "BITMAPS\xSIG.BMP";
            TOP PROMPT "Avance"; 
              ACTION  (oPosCmd:oBrwM:PageDown(),oPosCmd:oBrwM:Setfocus())
  ...
  DEFINE BUTTON oBtn;
          ...
          FILENAME "BITMAPS\xANT.BMP";
            TOP PROMPT "Anterior"; 
              ACTION  (oPosCmd:oBrwM:PageUp(),oPosCmd:oBrwM:Setfocus())
...
ENDIF
  ...
  DEFINE BUTTON oBtn;
          ...
          FILENAME "BITMAPS\xFIN.BMP";
            TOP PROMPT "Ultimo"; 
              ACTION  (oPosCmd:oBrwM:GoBottom(),oPosCmd:oBrwM:Setfocus())
  ...
  oPosCmd:oBrwM:SetColor(0,oPosCmd:nClrPane1)
  oPosCmd:oBrw:SetColor(0,oPosCmd:nClrPane1)

  EVAL(oPosCmd:oBrwM:bChange)
...
// ? oDp:cPictureDivisa,oPosCmd:nValCam,"oPosCmd:nValCam"

 @ 02+45,nColM SAY oPosCmd:oValCam PROMPT FDP(oPosCmd:nValCam,oDp:cPictureDivisa) OF oBar;
                   SIZE 110,20 PIXEL;
...
*/
FUNCTION BRWCHANGEF()
  LOCAL aLine   :=oPosCmd:oBrwM:aArrayData[oPosCmd:oBrwM:nArrayAt]
  LOCAL cMesa   :=aLine[1]
...
FUNCTION BRWREFRESCAR()
    ...
    IF Type("oPosCmd")="O" .AND. oPosCmd:oWnd:hWnd>0
      ...
      oPosCmd:LEERDATA(oPosCmd:cWhere_,oPosCmd:oBrwM,oPosCmd:cServer)
      oPosCmd:oWnd:Show()
      oPosCmd:oWnd:Maximize()
...
FUNCTION GetDataLic(lEmpty,cWhere,oBrw2)
  ...
  cSql:=[ SELECT MOV_ITEM,MOV_CODVEN,MOV_CODPER,MOV_CODIGO,INV_DESCRI,MOV_UNDMED,MOV_CANTID,MOV_PREDIV,MOV_MTOCLA,MOV_TIPIVA,MOV_MTODIV ]+;
        [ FROM DPMOVINV_CROSSD ]+;
...
FUNCTION RUNCLICKBRW2()
  ...
  oPosCmd:oBrwM:aArrayData:=ACLONE(aData)
  oPosCmd:oBrwM:GoBottom()

  EJECUTAR("BRWCALTOTALES",oPosCmd:oBrwM,.T.)
...
*/
FUNCTION CALTOTAL()
 LOCAL aData  :=oPosCmd:oBrwM:aArrayData,I,cTipIva
...
FUNCTION BRWCHANGE2()
   ...
   LOCAL aLineF  :=oPosCmd:oBrwM:aArrayData[oPosCmd:oBrwM:nArrayAt]
...
FUNCTION BRWDELITEM()
  ...
  IF !MsgNoYes("Desea Remover Item "+LSTR(oPosCmd:oBrwM:nArrayAt))
     RETURN .T.
  ENDIF


  aLine:=ACLONE(oPosCmd:oBrwM:aArrayData[oPosCmd:oBrwM:nArrayAt,4])
  ...
  ARREDUCE(oPosCmd:oBrwM:aArrayData,oPosCmd:oBrwM:nArrayAt)

  IF Empty(oPosCmd:oBrwM:aArrayData)
    oPosCmd:aData          :=ACLONE(oPosCmd:aVacio)
    oPosCmd:oBrwM:aArrayData:=ACLONE(oPosCmd:aVacio)
  ENDIF

  oPosCmd:oBrwM:nArrayAt:=MIN(oPosCmd:oBrwM:nArrayAt,LEN(oPosCmd:oBrwM:aArrayData))
  oPosCmd:oBrwM:nRowSel :=MIN(oPosCmd:oBrwM:nRowSel ,oPosCmd:oBrwM:nArrayAt)

  oPosCmd:oBrwM:Refresh(.F.)
...
FUNCTION GETDATAMESA(cMesa,dFecha)
  ...
  IF Empty(aDataM)
    aDataM:=EJECUTAR("SQLARRAYEMPTY",cSql,oDb)
...
FUNCTION CMDADDITEM(lAdd)
   ...
   LOCAL nPorIva,nCol:=IIF(oPosCmd:cZonaNL="N",3,5)
   ...
   IF lAdd
     ...
     nPorIva:=EJECUTAR("IVACAL",oPosCmd:INV_IVA,nCol,oDp:dFecha) 
     ...
     oPosCmd:oDPMOVINV:Replace("MOV_PREDIV",oPosCmd:PRE_PRECIO)
     oPosCmd:oDPMOVINV:Replace("MOV_PRECIO",ROUND(oPosCmd:PRE_PRECIO*oPosCmd:nValCam,2))
     oPosCmd:oDPMOVINV:Replace("MOV_TOTAL" ,ROUND(oPosCmd:PRE_PRECIO*oPosCmd:nValCam*oPosCmd:nCantid,2))
     oPosCmd:oDPMOVINV:Replace("MOV_MTODIV",oPosCmd:PRE_PRECIO*oPosCmd:nCantid)
     oPosCmd:oDPMOVINV:Replace("MOV_IVA"   ,nPorIva           )
     oPosCmd:oDPMOVINV:Replace("MOV_TIPIVA",oPosCmd:INV_IVA   )
...
*/
FUNCTION GUARDARDOC(cTipDoc)
  LOCAL dHasta :=oPosCmd:dHasta
  // LOCAL x   :=oPosCmd:oBrwM:aArrayData:=ACLONE(oPosCmd:aData)
  LOCAL y      :=oPosCmd:oBrwM:Refresh(.f.)
  LOCAL cDescri:="" // ALLTRIM(SQLGET("DPTIPDOCCLI","TDC_DESCRI","TDC_TIPO"+GetWhere("=",oPosCmd:cTipDoc)))
  LOCAL aData  :=ACLONE(oPosCmd:oBrw:aArrayData)
  LOCAL nMonto :=0
  LOCAL aTotal :=ATOTALES(aData)
  LOCAL cNumero,nCxC:=1 // todo se cobra
  LOCAL oMov,cWhere
  LOCAL nCol   :=IIF(oPosCmd:cZonaNL="N",3,5),nPorIva

  DEFAULT cTipDoc:="FAV"

  // en todos los casos realiza nota de entrega y luego del pago, realiza la factura.
  oPosCmd:cTipDoc:=cTipDoc
  cDescri:=ALLTRIM(SQLGET("DPTIPDOCCLI","TDC_DESCRI","TDC_TIPO"+GetWhere("=",oPosCmd:cTipDoc)))
  oPosCmd:cNumero:=EJECUTAR("DPDOCCLIGETNUM",cTipDoc)

  oPosCmd:CALTOTAL()
  oPosCmd:SETTIPDOC(oPosCmd:cTipDoc)

  IF Empty(oPosCmd:cCodCli)
    oPosCmd:oCodCli:VarPut(REPLI("0",10),.T.) // cliente CERO
  ENDIF
  ...
  oPosCmd:nTotal:=aTotal[11]

  IF !MsgNoYes("Desea "+IF(oPosCmd:nOption=1,"Crear","Modificar")+" "+cDescri+"  #"+oPosCmd:cNumero+" "+CRLF+"Total $"+oDp:cMonedaExt+" "+ALLTRIM(TRAN(oPosCmd:nTotal,"999,999,999.99")))
     RETURN .T.
  ENDIF
  ...
  cNumero:=EJECUTAR("DPDOCCLIGETNUM",cTipDoc)

  EJECUTAR("DPDOCCLICREA",NIL,cTipDoc,cNumero,oPosCmd:cCodCli,oDp:dFecha,oDp:cMonedaExt,"V",NIL,oPosCmd:nTotal,0,oPosCmd:nValCam,NIL,NIL,oPosCmd:oDocCli,"N",nCxC)

  // Cuerpo del Documento
  cWhere:="MOV_CODVEN"+GetWhere("=",oPosCmd:cMesa)

  oMov:=OpenTable("SELECT * FROM DPMOVINV_CROSSD WHERE "+cWhere,.T.)
  oMov:SetForeignkeyOff()
//  oMov:Browse()

  WHILE !oMov:Eof() 

     oPosCmd:oMovInv:AppendBlank()
     AEVAL(oMov:aFields,{|a,n| oPosCmd:oMovInv:Replace(a[1],oMov:FieldGet(a[1]))})
     oPosCmd:oMovInv:Replace("MOV_DOCUME",cNumero)
     oPosCmd:oMovInv:Replace("MOV_CODCTA",oPosCmd:cCodCli)
     oPosCmd:oMovInv:Replace("MOV_TIPDOC","NEN")
     oPosCmd:oMovInv:Replace("MOV_TIPDES",cTipDoc   ) // Destino
     oPosCmd:oMovInv:Replace("MOV_APLORG","V"       ) // Aplicación
     oPosCmd:oMovInv:Replace("MOV_FECHA" ,oDp:dFecha) // Fecha

     IF Empty(oPosCmd:oMovInv:MOV_TIPO)
        oPosCmd:oMovInv:Replace("MOV_TIPO","I") 
     ENDIF

     IF Empty(oPosCmd:oMovInv:MOV_TIPIVA)
        oPosCmd:oMovInv:Replace("MOV_TIPIVA","GN") 
     ENDIF

     IF Empty(oPosCmd:oMovInv:MOV_LISTA)
        oPosCmd:oMovInv:Replace("MOV_LISTA",oDp:cLista) 
     ENDIF


     // nPorIva:=EJECUTAR("IVACAL",oPosCmd:oMovInv:MOV_TIPIVA,nCol,oDp:dFecha) 
     //
     // oPosCmd:oMovInv:Replace("MOV_IVA"   ,nPorIva) 
     // oPosCmd:oMovInv:Replace("MOV_PRECIO",oMov:MOV_PREDIV/oPosCmd:nValCam)
     // oPosCmd:oMovInv:Replace("MOV_PREDIV",oMov:MOV_PRECIO) 
     // oPosCmd:oMovInv:Replace("MOV_TOTAL" ,oMov:MOV_TOTAL) 
     oPosCmd:oMovInv:Commit()
  ...
  SQLUPDATE("DPTIPDOCCLI","TDC_ACTIVO",.T.,"TDC_TIPO"+GetWhere("=","NEN"))

  EJECUTAR("DPDOCCLIIMP",oDp:cSucursal,"NEN",oPosCmd:cCodCli,cNumero,.T.,0,0,0,"V",0,NIL,NIL)

  //  ver el documento  EJECUTAR("DPFACTURAV","NEN",cNumero)

  EJECUTAR("DPRECIBODIV",oPosCmd:cCodCli) // Directo al Pago
  ...
  oDp:oCliRec:bAfterSave:=[EJECUTAR("DPDOCCLIPOSTPAG",]+;
                           GetWhere("",oPosCmd:cCodSuc)+[,]+;
                           GetWhere("",oPosCmd:cTipDoc)+[,]+;
                           GetWhere("",cNumero)+[,]+;
                           GetWhere("",oPosCmd:cSerFis)+[,]+;
                           GetWhere("",oPosCmd:cModFis)+[)]
